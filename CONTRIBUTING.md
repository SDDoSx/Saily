...
- `tools/build_chart.py` builds `site/chart-data.js` and `site/passage.js` from OSM extracts and a passage JSON.
...
- Safety first: any change to route geometry, TSS polygons, hazards or alert logic needs a source in the commit message
  and must keep `tools/build_chart.py`'s leg-clearance check clean.
...
## Pull requests
Run the unit and e2e tests, describe what you tested on a real phone if the change touches GPS, audio or the map,
